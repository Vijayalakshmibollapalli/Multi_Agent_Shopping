...
def clean_results(results: list) -> str:
    ...
    for x in results[:5]:
        cleaned.append({
            ...
            "content": str(x.get("content", "")).strip()[:1800]
        })

    # IMPORTANT:
    # Do NOT slice the final JSON string.
    # That was causing invalid JSON.
...
def search_products(query: str) -> str:
    return clean_results(
        search_web(
            f"{query} best laptops products exact models specifications India current",
            5
        )
    )
...
def search_reviews(query: str) -> str:
    return clean_results(
        search_web(
            f"{query} reviews ratings pros cons customer experience India",
            5
        )
    )
...
def search_comparison(query: str) -> str:
    return clean_results(
        search_web(
            f"{query} comparison alternatives competing products India",
            5
        )
    )
