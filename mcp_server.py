...
def search_web(query:str,max_results:int=3)->list:
    ...
    if not api_key:return [{"title":"ERROR","url":"","content":"TAVILY_API_KEY is missing on the remote MCP server."}]
    try:
...
def compact_results(results:list)->str:
    return json.dumps(results[:3],ensure_ascii=False)
@mcp.tool
def search_products(query:str)->str:
    return compact_results(search_web(f"{query} India best products specifications models 2026",3))
@mcp.tool
def search_prices(query:str)->str:
    return compact_results(search_web(f"{query} India current price Amazon India Flipkart Croma official 2026",3))
@mcp.tool
