...
def search_products(query:str)->str:
    return json.dumps(search_web(f"best exact product models for {query} India specifications price",8),ensure_ascii=False)
...
def search_prices(product:str)->str:
    return json.dumps(search_web(f'"{product}" exact model price India INR',5),ensure_ascii=False)
