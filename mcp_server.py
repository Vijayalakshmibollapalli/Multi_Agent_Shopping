...
def search_products(query: str) -> str:
    ...
    return compact_results(search_web(f"{query} India best products specifications models 2026", 3), 900)

@mcp.tool
def search_prices(query: str) -> str:
    """Find current product prices and availability in India."""
    return compact_results(search_web(f"{query} India current price Amazon India Flipkart Croma official 2026", 3), 900)

@mcp.tool
def search_reviews(query: str) -> str:
    """Find product reviews, pros, cons and customer feedback."""
    return compact_results(search_web(f"{query} India reviews pros cons customer feedback 2026", 3), 700)

@mcp.tool
def search_comparison(query: str) -> str:
    """Find product comparisons and alternatives."""
    return compact_results(search_web(f"{query} India comparison alternatives best products 2026", 3), 700)
